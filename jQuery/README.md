...
因为有些时候，在添加 css 样式的时候，不想添加为**行内样式**
...
<script>
  $(function() {
    ...
    $('#需要某个事件的元素的id').click(function () { // 添加一个事件
      // 给对应id的元素添加类，当添加多个类的时候，空格写上其他需要的类
      $('#需要变更样式的元素的id').addClass('已经写定声明的类的类名 第二个类的类名');
    })
  })
</script>
```
...
<script>
  $(function() {
    // 移除类 removeClass();
    $('#需要某个事件的元素的id').click(function () { // 添加一个事件
      // 给对应id的元素移除类，当添加多个类的时候，空格写上其他需要的类
      $('#需要变更样式的元素的id').removeClass('已经写定声明的类的类名 第二个类的类名');
    })
  })
</script>
```
...
<script>
  $(function() {
    // 显示 show;
    $('#需要某个事件的元素的id').click(function () { // 添加一个事件
      // 如果 show() 方法中没有参数，那就没有动画效果
      // $('#需要显示的元素的id').show();
      // 如果想到动画效果，就需要参数
      // 参数1：代表执行动画的时常 1.毫秒单位的数字 2.代表时常的字符串 fast，normal，slow
      // 参数2：代表动画执行完毕后的回调函数
      // $('#需要显示的元素的id').show(2000); // 持续 2秒 的动画
      // fast 200ms  normal 400ms  slow 600ms     如果字符串拼写错误，默认normal
      $('#需要显示的元素的id').show(2000, function () {
        alert('动画执行结束！');
      });
    })
  })
...
<script>
  $(function() {
    ...
    $('#需要某个事件的元素的id').click(function () { // 添加一个事件
      ...
      $('#需要切换的元素的id').toggle(1000, function () {
        alert('切换完毕！');
      });
    })
  })
