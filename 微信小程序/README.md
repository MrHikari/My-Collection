...
**注意**：
* 当设计稿外度为`page`（假设`width: page`）时，需要依据需求实现不同宽度的页面适配。
* 利用 `calc` 属性（*css*和*wxss*都支持的一个属性），注意 `750` 和 `rpx` 之间不要有空格，运算符的两边不要有空格。
...
calc示例：
```wxss
width: calc(750rpx*100/page);
```

#### 样式导入

*WXSS* 直接支持样式导入功能，也可以和 *less* 中的导入混用，使用 `@import` 属性可以导入外联样式表，**只支持相对路径**。

**示例**：
```wxss
@import "../../样式文件夹/样式文件.wxss"
```
...
**注意**：*button* 组件的附加功能 `open-type` 部分需要配合真机调试和企业级开发账号等权限。联系客服需要登记对应的客服wechat，并且需要真实的*appid*。